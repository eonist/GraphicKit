 ...
 * - Fixme: ⚠️️ why isnt this a decorator? if you find the answere then add to a - Note: in this description
 * - Fixme: ⚠️️ look into making ISIzeableGraohic and IPositionalGraphic again that extends the functionality you need but doesnt have the init stuff
 */
class GradientGraphic:SizeableDecorator/*<--Recently changed from GraphicDecoratable, and then changed to SizeableDec..was positionalDec etc, then changed back to positional, then changed to SizeableDecorator again after a bug surfaced*/ {//- Fixme: ⚠️️ probably should extend SizeableDecorator, so that we can resize the entire Decorator structure
    ...
    override func applyLineStyle() {
        ...
        if let lineStyle:GradientLineStylable = getGraphic().lineStyle as?  GradientLineStylable{
            ...
            //- Fixme: ⚠️️ The above isn't totally correct, use the outline-stroke method and then get the boundingbox from that, think different caps etc
