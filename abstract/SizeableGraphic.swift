...
class SizeableGraphic:PositionalGraphic,Sizable {
    var size: CGSize
    init(_ position: CGPoint, _ size: CGSize, _ decoratable:GraphicDecoratableKind = BaseGraphic(FillStyle(NSColor.red))) {//- Fixme: ⚠️️add the last arg through an extension?
        self.size = size
 ...
 * - Fixme: ⚠️️ ⚠️️ Add explination and example code for each init, and maybe move to it's own class
 */
extension SizeableGraphic{
    ...
    /**
     * - Fixme: ⚠️️ ⚠️️ is this init really needed?
     */
    ...
    /**
     * - Fixme: ⚠️️ ⚠️️ I think the two bellow init methods can be merged into one, do it and test explorer if it works
     */
